...
async function loadSessionNPCs() {
  ...
  try {
    const response = await fetch(`/api/npcs/${currentUserId}`);
    if (!response.ok) {
      if (response.status === 404) {
        showMessage(
          "Game session not found. Loading base NPCs instead.",
          "error"
        );
        loadBaseNPCs();
        return;
    ...
    renderNPCs();
    showMessage(
      `Loaded ${currentNPCs.length} NPCs from game session`,
      "success"
    );
  } catch (error) {
...
async function loadPermanentNPCs() {
  try {
    const response = await fetch("/api/database/npcs");
    if (response.ok) {
      const data = await response.json();
      if (data.npcs && data.npcs.length > 0) {
        permanentNPCs = data.npcs;
        currentNPCs = JSON.parse(JSON.stringify(permanentNPCs)); // Deep copy
        renderNPCs();
        showMessage(
          `✅ Loaded ${currentNPCs.length} permanent NPCs from database`,
          "success"
        );
      } else {
        loadBaseNPCs();
        showMessage(
          "ℹ️ No permanent NPCs saved in database. Loaded base NPCs. Edit and save to make permanent.",
          "info"
        );
      }
    } else {
      ...
      loadBaseNPCs();
      showMessage(
        "ℹ️ Database unavailable. Loaded base NPCs. Edit and save to make permanent.",
        "info"
      );
    }
  } catch (error) {
...
async function savePermanentNPCs() {
  try {
    ...
    const response = await fetch("/api/database/npcs", {
      ...
      body: JSON.stringify({ npcs: permanentNPCs }),
    });

    if (response.ok) {
      showMessage(
        `✅ Saved ${permanentNPCs.length} NPCs to database! This will apply to all new games.`,
        "success"
      );
    } else {
      ...
      showMessage(
        `❌ Failed to save NPCs to database: ${
...
function createNPCCard(npc) {
  ...
  card.innerHTML = `
        ...
        <div class="form-group">
            <label>Tribe</label>
            <input type="text" id="tribe-${npc.id}" value="${
    npc.tribe
  }" disabled>
        </div>
        ...
        <div class="form-group">
            ...
            <select id="disposition-${npc.id}">
                <option value="Friendly" ${
                  npc.disposition === "Friendly" ? "selected" : ""
                }>Friendly</option>
                <option value="Neutral" ${
                  npc.disposition === "Neutral" ? "selected" : ""
                }>Neutral</option>
                <option value="Hostile" ${
                  npc.disposition === "Hostile" ? "selected" : ""
                }>Hostile</option>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" id="relationship-${npc.id}" value="${
    npc.relationship
  }" min="0" max="100">
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" id="growth_path-${npc.id}" value="${
    npc.growth_path
  }">
        </div>
        ...
        <div class="form-group">
            <label>Level</label>
            <input type="number" id="level-${npc.id}" value="${
    npc.level || 1
  }" min="1">
        </div>
        ...
        <div class="form-group">
            <label>XP</label>
            <input type="number" id="xp-${npc.id}" value="${
    npc.xp || 0
  }" min="0">
        </div>
        ...
        <div class="form-group">
            ...
            <div class="stats-grid">
                <div class="stat-input">
                    <label>STR</label>
                    <input type="number" id="stat-str-${npc.id}" value="${
    npc.stats?.str || 3
  }" min="1">
                </div>
                <div class="stat-input">
                    <label>DEX</label>
                    <input type="number" id="stat-dex-${npc.id}" value="${
    npc.stats?.dex || 3
  }" min="1">
                </div>
                <div class="stat-input">
                    <label>WIS</label>
                    <input type="number" id="stat-wis-${npc.id}" value="${
    npc.stats?.wis || 3
  }" min="1">
                </div>
                <div class="stat-input">
                    <label>CHA</label>
                    <input type="number" id="stat-cha-${npc.id}" value="${
    npc.stats?.cha || 3
  }" min="1">
                </div>
                <div class="stat-input">
                    <label>LCK</label>
                    <input type="number" id="stat-luck-${npc.id}" value="${
    npc.stats?.luck || 3
  }" min="1">
                </div>
            </div>
        ...
        <div class="button-group">
            <button class="btn btn-primary btn-small" onclick="updateNPC('${
              npc.id
            }')">
                💾 Save Changes
            </button>
...
async function updateNPC(npcId) {
  ...
  const updates = {
    ...
    relationship: parseInt(
      document.getElementById(`relationship-${npcId}`).value
    ),
  ...
  // If we have a user, also update it
  if (currentUserId) {
    try {
      const response = await fetch(`/api/npcs/${currentUserId}/${npcId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  showMessage(
    `✅ ${npc.name} updated! (Save Permanent to make it permanent)`,
    "success"
  );
  renderNPCs();
}
...
async function growNPC(npcId, xpAmount) {
  ...
  try {
    const response = await fetch(`/api/npcs/${currentUserId}/${npcId}/grow`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (data.leveledUp) {
      showMessage(
        `🌟 ${npc.name} leveled up! Now level ${data.npc.level}`,
        "success"
      );
    } else {
